...
public final class Mod003MethodReferences {
    ...
    */
    static void instanceOnType() {
        ...
        // String::compareToIgnoreCase  =  (a, b) -> a.compareToIgnoreCase(b)  -- a two-argument
        // unbound reference, which is exactly the shape of Comparator<String>.
    ...
    */
    ...
    writing a single block lambda. Comparator has its own, much richer set of combinators -- Mod004 is devoted
    to it.
    */
    static void composition() {
        ...
        // Function.identity() -- structurally required, semantically a no-op. The classic use is
        // toMap(keyFn, identity()): key by something, keep the whole element as the value.
        var byFirstLetter = WORDS.stream()
                .filter(notBlank)
                .collect(Collectors.toMap(s -> s.substring(0, 1), Function.identity(), (a, b) -> a));
        System.out.println("  toMap(.., identity()) -> " + byFirstLetter);
    }
