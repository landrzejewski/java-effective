...
- Without `uses`, `ServiceLoader.load(...)` **throws** `ServiceConfigurationError`
  ("… does not declare `uses`") when the caller is an explicit named module —
  it does not merely come back empty. Callers on the classpath (the unnamed
  module) and automatic modules are exempt from the check.
