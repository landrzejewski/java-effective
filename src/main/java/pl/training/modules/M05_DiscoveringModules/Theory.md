...
- Prints the module's `module-info` summary: requires, exports, opens, uses,
  provides. For an automatic module the first line ends with a bare `automatic`
  marker (no brackets):
  `zoo.legacy file:///…/mods/zoo.legacy.jar automatic`
- A qualified export gets its own line, **prefixed** with `qualified`:
  `qualified exports zoo.animal.talks.content to zoo.staff`
