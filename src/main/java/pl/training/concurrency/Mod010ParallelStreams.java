...
public final class Mod010ParallelStreams {
    ...
    */
    ...
    - forEach does not guarantee order, even on a sequential stream's parallel fork. Use forEachOrdered if you need
      source order.
    ...
    */
    ...
    - A stateful lambda inside forEach on a parallel stream produces lost updates (if the structure is not
      ...
      mid-write).
    - The fix is usually to express the same logic with collect, which the runtime parallelises correctly.
    */
    ...
    - For string concatenation, do not use reduce("", String::concat) (O(n²) allocations and not associative for the
      empty-string identity in subtle ways). Use Collectors.joining instead.
    */
    ...
    - For grouping reductions on parallel streams, prefer groupingByConcurrent — it tends to be measurably faster on
      multi-core machines.
    */
