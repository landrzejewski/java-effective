...
In Java, every functional interface is first-class — a `Function<T, R>`,
`Predicate<T>`, `BiFunction<T, U, R>` is just a value. Lambdas and method
references make first-class functions cheap to write.

This module focuses on what you can *do* with that capability: pass
behaviour, build behaviour from smaller pieces, configure functions
from data.
*/
...
A *higher-order function* (HOF) takes a function as a parameter, returns
a function, or both.

- `transform(list, fn)` — takes a transformation as a parameter.
- `select(list, predicate)` — same idea for filtering.
- `whenMatched(predicate, handler)` — returns a function configured by
  inputs.
- The JDK's `Stream` API is built entirely on HOFs (`map`, `filter`,
  `reduce`).

HOFs let one piece of code drive many behaviours; the *what* and the
*how* can move independently.
*/
...
*Currying* converts an N-argument function into N nested 1-argument
functions:
...
Curried functions compose more easily because every step is a
`Function<X, Y>` — the standard interface that all the JDK combinators
work with. They also make partial application natural (next section).

The cost: more allocations and more nested types in error messages.
Curry on demand, do not write everything in curried form.
*/
...
producing a function that takes the remaining ones:
...
The currying form makes partial application a one-step operation:
`addFive = curry(add).apply(5)`.

Use cases: build configuration-style helpers (`logAt(LEVEL_ERROR, msg)`),
specialise generic operations (`replaceAll(map, "x" -> _)`),
adapt one functional interface to another.
*/
...
- `Function.identity()` — `x -> x`. The neutral element for composition;
  useful when an API requires a `Function` but you have nothing to do.

Compose pure functions into pipelines that read in the order in which
data flows. The composed function is itself a value — store it, log it,
test it.
*/
...
1. As a chain of composed `Function`s.
2. As an imperative for-loop with intermediate variables.

The two outputs are compared element-by-element to confirm the composed
pipeline matches the imperative reference.
*/
