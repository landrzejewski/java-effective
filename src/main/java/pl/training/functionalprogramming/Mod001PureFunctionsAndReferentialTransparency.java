...
- Testability — no test fixtures, no mocking the clock, no dependency injection setup. The function takes inputs,
  you assert outputs.
- Parallelisation — a pure function has no shared state to race over. The runtime can split work across cores or
  hosts safely.
- Caching / memoisation — same inputs → same output, so a cache hit can replace the call entirely.
- Refactoring — equational reasoning. A pure expression can be inlined, factored out, or reordered without changing
  meaning.
- Reasoning — when reading a pure function, you don't have to know what other code is doing concurrently or in the
  past.
*/
...
public final class Mod001PureFunctionsAndReferentialTransparency {
    ...
    - Determinism — same arguments always produce the same result. No hidden inputs (clock, random, environment,
      file system).
    - No observable side effects — calling it does not modify global state, allocate resources visible to the
      outside world, mutate its arguments, throw exceptions for non-error conditions, or print to a stream.

    A pure function is a mathematical function: a mapping from its inputs to its outputs. Everything you need to
    know about it is in its signature and its return value.

    Referential transparency

    A piece of code is referentially transparent (RT) when it can be replaced with the value it produces without
    changing program meaning.

    let x = pure(args)
    use(x, x, x)        ≡   use(pure(args), pure(args), pure(args))

    Pure functions are referentially transparent by construction. Impure functions are not — replacing now() with
    the value it returned earlier gives you a different program.

    RT is the property that makes equational reasoning possible: refactoring, inlining, caching, parallelisation,
    and unit testing all rely on it.
    */
    ...
    - Mutation of arguments — list.add(...) on a parameter you did not own.
    - Field mutation — counters, caches, "last value" state.
    - Hidden inputs — LocalDate.now(), Math.random(), environment variables, file reads.
    - Hidden outputs — System.out.println, logging, network calls.
    - Throwing exceptions for ordinary conditions — replaces "function returns a value" with "function returns a
      value or unwinds the stack".

    None of this is automatically wrong. The point is that each breakage costs reasoning power, so a function that
    needs only purity should not incur it.
    */
    ...
    - A class is immutable when its observable state never changes after construction. record, final fields,
      defensive copies of mutable inputs, no setters, no mutable references leaking out.
    - An immutable input cannot be mutated by callees, so callers and implementations can pass references freely
      without defensive copying.
    - "Copy-with-changes" idiom: instead of obj.setX(v) you write a constructor or a with-method that returns a
      new instance.
    */
    ...
    - Impure: mutates the input list and the orders in place; calling it twice with the same input applies the
      discount twice.
    - Pure: returns a fresh list of fresh Order instances; calling it twice yields the same result; the input list
      is bit-for-bit unchanged.

    The main method runs both, and a self-check confirms the pure version left the input untouched.
    */
